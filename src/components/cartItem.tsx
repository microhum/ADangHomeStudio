...
const ShoppingCartItem: FC<ShoppingCartProps> = ({ item }) => {
  ...
  return (
    <button
      id={id}
      onClick={handleClick}
      className={`relative flex flex-col justify-center gap-y-4 items-center md:flex-row md:justify-around w-5/6 px-2 py-5 bg-white rounded-xl border-4 transition-all duration-400 ${
        selected?.id == id
          ? "border-amber-600"
    ...
    >
      ...
      {/* description */}
      <p className="flex md:w-8/12 text-sm lg:text-lg font-light text-justify px-8">{description}</p>
