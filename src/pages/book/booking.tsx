...
const Booking = () => {
  const items = [
    { id: 1, description: "Good Course You should go !", name: 'Item 1', price: 2000 ,place_left:0},
    { id: 2, description: "Good Course You should go !", name: 'Item 2', price: 2000 ,place_left:2},
    { id: 3, description: "Good Course You should go !", name: 'Item 3', price: 2000 ,place_left:1},
  ];
  return (
    <>
      ...
      <div className="flex flex-col  gap-8 items-center bg-cream w-full h-screen">
        ...
        {items.map((item) => (
            <ShoppingCartItem id={item.id} description={item.description} name={item.name} price={item.price} place_left={item.place_left}/>
        ))}
